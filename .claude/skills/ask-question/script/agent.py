    ...
    def __init__(
        ...
        **kwargs
        ...
        if knowledge_base_path is None:
            ...
            knowledge_base_path = "http://localhost:19530"
        ...
        logger.info(f"连接Milvus数据库: {milvus_db_path}")
        self.milvus_client = MilvusClient(uri=milvus_db_path , grpc_options={"keepalive_time": _GRPC_MAX_MS, "keepalive_timeout": "20000", "keepalive_permit_without_calls": "0", "http2_min_recv_ping_interval_without_data": _GRPC_MAX_MS, "http2_min_ping_interval_without_data": _GRPC_MAX_MS})
...
# 3. 缓冲时间 (Sleep)：给底层的分布式/本地引擎留一点点同步索引的时间
            #print("⏳ 正在等待数据刷入检索引擎...")
            #time.sleep(2)
